...
const legalAs = (
  ...
  if (isOneRankMove(move, toPiece)) {
    return isGettingPromoted(move.to, move.piece.side) ? 'promote' : 'simple'
  }
...
const legalMoves = (
  ...
  if (!board.getOccupant(squareInFront)) {
    resolvable.push({
      ...
      type: isGettingPromoted(squareInFront, piece.side) ? 'promote' : 'simple'
    })
  ...
  attackableSquares().forEach((pos) => {
    if (isOpponent(board.getOccupant(pos), piece.side)) {
      resolvable.push({
        ...
        type:  isGettingPromoted(squareInFront, piece.side) ? 'capturePromote' : 'capture'
      })
    }
  })
  return resolvable
