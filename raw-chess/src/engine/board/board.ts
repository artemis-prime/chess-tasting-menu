...
import type IsCaptureFn from '../is-capture-fn'

import Tracking, { type TrackingSnapshot } from './tracking'
import BoardSquares, { type SquaresSnapshot } from './board-squares'
...
class BoardImpl implements BoardInternal {
  ...
  applyAction(r: MoveRecord, mode: HistoryMode): void {
    ...
    if (r.type === 'castle') {
      ...
      if (r.type.includes('capture')) {
        this._sq(r.move.to).occupant = r.captured!
      }
      if (r.type.includes('romote')) {
  ...
  private _track(r: MoveRecord, mode: HistoryMode): void {
    ...
    if (r.type === 'castle') {
      ...
      if (r.type === 'simple' || r.type.includes('capture')) {
